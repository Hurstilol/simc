...
void windwalker( player_t* p )
{
  ...
  // ST / Cleave
  st_cleave->add_action( "faeline_stomp,if=combo_strike&(!talent.faeline_harmony|!debuff.fae_exposure_damage.up)", "ST Priority (<3 Targets)" );
  ...
  fallthru->add_action( "tiger_palm,target_if=min:debuff.mark_of_the_crane.remains+(debuff.skyreach_exhaustion.up*20),if=combo_strike&chi.max-chi>=(2+buff.power_strikes.up)&buff.storm_earth_and_fire.down" );
  ...
  serenity->add_action( "faeline_stomp,if=combo_strike&(!talent.faeline_harmony|!debuff.fae_exposure_damage.up)" );
